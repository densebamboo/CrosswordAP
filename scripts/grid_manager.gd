...
const DEBUG_CELL_LOCK := preload("res://scripts/debug_cell_locking.gd")
const AP_DEBUG_LOG := preload("res://scripts/ap_debug_log.gd")
const AP_DEBUG_HARNESS := preload("res://scripts/ap_debug_harness.gd")
const DIRECTIONS := {
	"across": Vector2i(0, 1),
	"down": Vector2i(1, 0),
}
...
const DEBUG_COLOR_INDICATOR := false
const DEBUG_AP_LAYOUT := false
const DEBUG_SETUP := false
const DEBUG_AP_SAVE := false
const DEBUG_AP_PROCESS := false
const DEBUG_CLUE_DUMP := false
...
@export var enable_ap_debug_hotkeys: bool = true
...
var _last_logged_save_signature: String = ""
...
func _debug_log_cell_click(pos: Vector2i, has_active: bool, is_across: bool, is_down: bool, is_intersection: bool, has_num_label: bool, num_label: String, options: Dictionary) -> void:
	var num_info := ""
	if is_across and is_down:
		num_info = "%d across, %d down" % [options["across"].get("number", 0), options["down"].get("number", 0)]
	elif is_across:
		num_info = "%d across" % options["across"].get("number", 0)
	elif is_down:
		num_info = "%d down" % options["down"].get("number", 0)
	var clue_info := ""
...
func _ready() -> void:
	AP_DEBUG_LOG.start_session("grid_manager_ready")
	...
	regenerate_button = get_node_or_null("Margin/Layout/CenterPanel/Controls/OfflineSection/OfflineControls/RegenerateButton")
	...
	if regenerate_button == null:
		if DEBUG_SETUP: print("[DEBUG] Failed to find RegenerateButton at expected path")
		# Try to find it by searching the scene tree
		regenerate_button = _find_button_by_text("Generate")
		if regenerate_button != null:
			if DEBUG_SETUP: print("[DEBUG] Found RegenerateButton by searching for 'Generate' text")
	else:
		if DEBUG_SETUP: print("[DEBUG] Found RegenerateButton at expected path")
	easy_words_toggle = get_node_or_null("Margin/Layout/CenterPanel/Controls/OfflineSection/OfflineControls/EasyWordsToggle")
	hard_words_toggle = get_node_or_null("Margin/Layout/CenterPanel/Controls/OfflineSection/OfflineControls/HardWordsToggle")
	ap_host_input = get_node_or_null("Margin/Layout/CenterPanel/Controls/APSection/APHostInput")
	ap_name_input = get_node_or_null("Margin/Layout/CenterPanel/Controls/APSection/APControls/APNameInput")
	ap_port_input = get_node_or_null("Margin/Layout/CenterPanel/Controls/APSection/APControls/APPortInput")
	ap_password_input = get_node_or_null("Margin/Layout/CenterPanel/Controls/APSection/APControls/APPasswordInput")
	ap_connect_button = get_node_or_null("Margin/Layout/CenterPanel/Controls/APSection/APControls/APConnectButton")
	if ap_client == null:
	...
	across_label = get_node_or_null("Margin/Layout/AcrossPanel/AcrossLabel") as Label
	down_label = get_node_or_null("Margin/Layout/DownPanel/DownLabel") as Label
	...
	_connect_ap_input_submit(ap_host_input)
	_connect_ap_input_submit(ap_port_input)
	_connect_ap_input_submit(ap_name_input)
	_connect_ap_input_submit(ap_password_input)
	_load_ap_settings()
	...
	if regenerate_button.pressed.is_connected(_on_regenerate_pressed):
		if DEBUG_SETUP: print("[DEBUG] Generate button already connected")
	else:
		if DEBUG_SETUP: print("[DEBUG] Connecting generate button")
		regenerate_button.pressed.connect(_on_regenerate_pressed)
	
	# Double-check the connection worked
	if regenerate_button.pressed.is_connected(_on_regenerate_pressed):
		if DEBUG_SETUP: print("[DEBUG] Generate button connection confirmed")
	else:
		push_error("Failed to connect generate button!")
	
	# Verify button state
	if DEBUG_SETUP: print("[DEBUG] Generate button disabled: %s, visible: %s" % [str(regenerate_button.disabled), str(regenerate_button.visible)])
...
func _on_ap_client_state_changed(new_state: int) -> void:
	AP_DEBUG_LOG.event("GridManager", "ap_client_state_changed", {
		"new_state": new_state,
...
func _on_ap_client_failed(reason: String) -> void:
	push_warning(reason)
	AP_DEBUG_LOG.event("GridManager", "ap_client_failed", {
		"reason": reason,
		"current_mode": current_mode,
		"puzzle_ready": _puzzle_ready,
	})
...
func _on_ap_slot_data_received(data: Dictionary) -> void:
	if _save_pending and current_mode == PlayMode.ARCHIPELAGO and not ap_slot_data.is_empty() and not player_grid.is_empty():
		_save_progress_now()
	ap_slot_data = data.duplicate(true)
	AP_DEBUG_LOG.event("GridManager", "slot_data_received", {
		"keys": ap_slot_data.keys(),
		"total_words": ap_slot_data.get("total_words", null),
		"initial_clues": ap_slot_data.get("initial_clues", null),
		"puzzle_seed": ap_slot_data.get("puzzle_seed", null),
		"entry_count": ap_slot_data.get("entry_count", null),
	})
	if current_mode == PlayMode.ARCHIPELAGO:
...
func _apply_archipelago_slot_data() -> void:
	if ap_slot_data.is_empty():
		AP_DEBUG_LOG.event("GridManager", "slot_layout_waiting")
		_update_active_clue_message("Waiting for Archipelago slot data...", false)
		return
	_reset_archipelago_progress_tracking()
	AP_DEBUG_LOG.event("GridManager", "slot_layout_apply_start", {
		"total_words": ap_slot_data.get("total_words", null),
		"initial_clues": ap_slot_data.get("initial_clues", null),
		"puzzle_seed": ap_slot_data.get("puzzle_seed", null),
		"entry_count": ap_slot_data.get("entry_count", null),
	...
	if typeof(layout_variant) != TYPE_DICTIONARY:
		AP_DEBUG_LOG.event("GridManager", "slot_layout_invalid", {"reason": "missing_puzzle_layout"})
	...
	if converted_layout.is_empty():
		AP_DEBUG_LOG.event("GridManager", "slot_layout_invalid", {"reason": "conversion_failed"})
		push_warning("Failed to convert Archipelago puzzle layout.")
		return
	var entry_list_variant: Variant = converted_layout.get("entries", [])
	var entry_count: int = 0
	if DEBUG_AP_LAYOUT and typeof(entry_list_variant) == TYPE_ARRAY:
	...
	AP_DEBUG_LOG.event("GridManager", "slot_layout_applied", {
		"entry_count": entry_count,
		"initial_reveal": initial_reveal_override,
		"restored": restored,
		"hidden_queue": hidden_entry_queue.size(),
		"revealed": revealed_entry_count,
	})
	if restored:
		if DEBUG_AP_SAVE: print_debug("[APSave] Restored progress for %s" % _get_current_puzzle_id())
		# Hints were restored from save, don't reset them
	else:
		if DEBUG_AP_SAVE: print_debug("[APSave] No saved progress applied for %s" % _get_current_puzzle_id())
		...
		if ap_client != null:
			ap_client.restore_item_tracking(-1, total_direct_clues_received, total_letter_hint_items_received)
		_update_letter_hint_counter()
		if DEBUG_AP_PROCESS: print_debug("[LetterHint] New game - reset to starting_hints=%d" % starting_hints)
	...
	_update_letter_hint_counter()
	AP_DEBUG_LOG.event("GridManager", "puzzle_ready", {
		"entry_count": entry_count,
		"hidden_queue": hidden_entry_queue.size(),
		"revealed": revealed_entry_count,
	...
	_resend_unsent_solved_archipelago_checks("puzzle_ready")

func _reset_archipelago_progress_tracking() -> void:
	claimed_location_ids.clear()
	sent_checks_for_entries.clear()
	processed_clue_locations.clear()
	total_direct_clues_received = 0
...
func _flush_pending_archipelago_items() -> void:
	if not _puzzle_ready:
		return
	AP_DEBUG_LOG.event("GridManager", "flush_pending_items_start", {
		"pending_color_indicator": _pending_color_indicator,
		"pending_direct_total": _pending_direct_clue_total,
		"pending_direct_new": _pending_direct_clue_new,
		"pending_clue_locations": _pending_clue_locations.keys(),
		"pending_letter_hints": _pending_letter_hints,
	})
	...
	_update_letter_hint_counter()
	AP_DEBUG_LOG.event("GridManager", "flush_pending_items_done", {
		"hidden_queue": hidden_entry_queue.size(),
		"revealed": revealed_entry_count,
		"letter_hints_available": letter_hints_available,
		"total_direct_clues_received": total_direct_clues_received,
...
func debug_run_received_items_sequence() -> void:
	if current_mode != PlayMode.ARCHIPELAGO or not _puzzle_ready:
		AP_DEBUG_LOG.event("GridManager", "debug_harness_not_ready", {
			"action": "received_items_sequence",
			"current_mode": current_mode,
			"puzzle_ready": _puzzle_ready,
		})
		_update_active_clue_message("Debug: connect to AP puzzle first", false)
		return
	AP_DEBUG_HARNESS.run_received_items_sequence(self)
	_update_active_clue_message("Debug: received items sequence", false)

func debug_run_retroactive_items_sequence() -> void:
	if current_mode != PlayMode.ARCHIPELAGO or not _puzzle_ready:
		AP_DEBUG_LOG.event("GridManager", "debug_harness_not_ready", {
			"action": "retroactive_items_sequence",
			"current_mode": current_mode,
			"puzzle_ready": _puzzle_ready,
		})
		_update_active_clue_message("Debug: connect to AP puzzle first", false)
		return
	AP_DEBUG_HARNESS.run_retroactive_items_sequence(self)
	_update_active_clue_message("Debug: retroactive items sequence", false)

func debug_audit_unsent_solved_entries() -> void:
	AP_DEBUG_HARNESS.audit_unsent_solved_entries(self)
	_update_active_clue_message("Debug: unsent check audit", false)

func debug_resend_unsent_solved_entries() -> void:
	_resend_unsent_solved_archipelago_checks("debug_hotkey")
	_update_active_clue_message("Debug: resent unsent checks", false)

func debug_simulate_ap_connection_drop() -> void:
	if current_mode != PlayMode.ARCHIPELAGO or not _puzzle_ready:
		AP_DEBUG_LOG.event("GridManager", "debug_harness_not_ready", {
		...
		return
	if ap_client == null:
		AP_DEBUG_LOG.event("GridManager", "debug_connection_drop_failed", {"reason": "missing_ap_client"})
		return
	ap_client.debug_simulate_connection_drop()
	AP_DEBUG_LOG.event("GridManager", "debug_connection_drop_kept_puzzle", {
		"mode": current_mode,
		"puzzle_ready": _puzzle_ready,
	...
	_update_active_clue_message("Debug: AP socket dropped; puzzle kept", false)

func _input(event: InputEvent) -> void:
	if not enable_ap_debug_hotkeys:
		return
	...
	match key_event.keycode:
		KEY_6:
			AP_DEBUG_LOG.event("GridManager", "debug_hotkey_pressed", {"key": "Shift+6", "action": "resend_unsent_solved_entries"})
			debug_resend_unsent_solved_entries()
			get_viewport().set_input_as_handled()
		KEY_7:
			AP_DEBUG_LOG.event("GridManager", "debug_hotkey_pressed", {"key": "Shift+7", "action": "simulate_connection_drop"})
			debug_simulate_ap_connection_drop()
			get_viewport().set_input_as_handled()
		KEY_8:
			AP_DEBUG_LOG.event("GridManager", "debug_hotkey_pressed", {"key": "Shift+8", "action": "received_items_sequence"})
			debug_run_received_items_sequence()
			get_viewport().set_input_as_handled()
		KEY_9:
			AP_DEBUG_LOG.event("GridManager", "debug_hotkey_pressed", {"key": "Shift+9", "action": "retroactive_items_sequence"})
			debug_run_retroactive_items_sequence()
			get_viewport().set_input_as_handled()
		KEY_0:
			AP_DEBUG_LOG.event("GridManager", "debug_hotkey_pressed", {"key": "Shift+0", "action": "unsent_solved_entries_audit"})
...
func _save_progress_now() -> void:
	...
	if snapshot.is_empty():
		return
	if DEBUG_AP_SAVE: print_debug("[APSave] Snapshot revealed=%s solved=%s hidden=%s" % [str(snapshot.get("revealed_ids", [])), str(snapshot.get("solved_ids", [])), str(snapshot.get("hidden_queue", []))])
	var save_signature := _build_save_log_signature(snapshot)
	var should_log_save := DEBUG_AP_SAVE or save_signature != _last_logged_save_signature
	if should_log_save:
		_last_logged_save_signature = save_signature
		AP_DEBUG_LOG.event("GridManager", "save_progress", {
			"path": _get_save_path(puzzle_id),
			"revealed_count": (snapshot.get("revealed_ids", []) as Array).size(),
			"solved_count": (snapshot.get("solved_ids", []) as Array).size(),
			"hidden_queue_count": (snapshot.get("hidden_queue", []) as Array).size(),
			"claimed_location_count": (snapshot.get("claimed_locations", []) as Array).size(),
			"sent_check_entry_count": (snapshot.get("sent_check_entries", []) as Array).size(),
			"claimed_locations": snapshot.get("claimed_locations", []),
			"sent_check_entries": snapshot.get("sent_check_entries", []),
			"ap_last_item_index": snapshot.get("ap_last_item_index", -1),
			"total_direct_clues_received": snapshot.get("total_direct_clues_received", 0),
			"total_letter_hint_items_received": snapshot.get("total_letter_hint_items_received", 0),
		})
	...
	if DEBUG_AP_SAVE: print_debug("[APSave] Stored progress %s" % _get_save_path(puzzle_id))

func _build_save_log_signature(snapshot: Dictionary) -> String:
	return JSON.stringify({
		"revealed": snapshot.get("revealed_ids", []),
		"solved": snapshot.get("solved_ids", []),
		"hidden": snapshot.get("hidden_queue", []),
		"claimed": snapshot.get("claimed_locations", []),
		"sent": snapshot.get("sent_check_entries", []),
		"last_item": snapshot.get("ap_last_item_index", -1),
		"direct": snapshot.get("total_direct_clues_received", 0),
		"hints": snapshot.get("total_letter_hint_items_received", 0),
	})
...
func _restore_saved_progress() -> bool:
	var puzzle_id := _get_current_puzzle_id()
	if puzzle_id.is_empty():
		if DEBUG_AP_SAVE: print_debug("[APSave] No puzzle id; skip restore")
		AP_DEBUG_LOG.event("GridManager", "restore_skip", {"reason": "empty_puzzle_id"})
		return false
	var path := _get_save_path(puzzle_id)
	if not FileAccess.file_exists(path):
		if DEBUG_AP_SAVE: print_debug("[APSave] No existing save for %s" % path)
		AP_DEBUG_LOG.event("GridManager", "restore_skip", {"reason": "missing_save", "path": path})
		return false
	var file := FileAccess.open(path, FileAccess.READ)
	if file == null:
		if DEBUG_AP_SAVE: print_debug("[APSave] Failed to open save for reading: %s" % path)
		AP_DEBUG_LOG.event("GridManager", "restore_failed", {"reason": "open_failed", "path": path})
		return false
	var contents := file.get_as_text()
	var parsed: Variant = JSON.parse_string(contents)
	if typeof(parsed) != TYPE_DICTIONARY:
		if DEBUG_AP_SAVE: print_debug("[APSave] Save file malformed for %s" % path)
		AP_DEBUG_LOG.event("GridManager", "restore_failed", {"reason": "malformed_json", "path": path})
		return false
...
func _array_to_id_set(array: Array) -> Dictionary:
	var set: Dictionary = {}
	for value in array:
		var id := _coerce_int(value)
		if id != 0 or str(value) == "0":
			set[id] = true
	return set

func _get_max_direct_clue_items() -> int:
	var slot_clue_count_variant: Variant = ap_slot_data.get("clue_item_count", null)
	if typeof(slot_clue_count_variant) in [TYPE_INT, TYPE_FLOAT]:
		return max(0, int(slot_clue_count_variant))
	var entry_total := across_entries.size() + down_entries.size()
	return max(0, entry_total - initial_reveal_override)

func _apply_saved_progress(state: Dictionary) -> bool:
	if state.get("version", 0) != SAVE_VERSION:
		if DEBUG_AP_SAVE: print_debug("[APSave] Save version mismatch (found %s)" % str(state.get("version", "?")))
		AP_DEBUG_LOG.event("GridManager", "restore_failed", {
			"reason": "version_mismatch",
			"found": state.get("version", null),
			"expected": SAVE_VERSION,
		})
		return false
	var expected_seed := int(ap_slot_data.get("puzzle_seed", 0))
	if expected_seed != 0 and expected_seed != int(state.get("puzzle_seed", -1)):
		if DEBUG_AP_SAVE: print_debug("[APSave] Puzzle seed mismatch expected=%d saved=%d" % [expected_seed, int(state.get("puzzle_seed", -1))])
		AP_DEBUG_LOG.event("GridManager", "restore_failed", {
			"reason": "seed_mismatch",
			"expected": expected_seed,
			"saved": int(state.get("puzzle_seed", -1)),
		})
		return false
	var saved_grid = state.get("player_grid", [])
	if typeof(saved_grid) != TYPE_ARRAY:
		if DEBUG_AP_SAVE: print_debug("[APSave] Saved grid missing or invalid")
		AP_DEBUG_LOG.event("GridManager", "restore_failed", {"reason": "invalid_grid"})
		return false
	var saved_rows: int = saved_grid.size()
	if saved_rows != player_grid.size():
		if DEBUG_AP_SAVE: print_debug("[APSave] Saved grid row mismatch saved=%d expected=%d" % [saved_rows, player_grid.size()])
		AP_DEBUG_LOG.event("GridManager", "restore_failed", {
			"reason": "row_mismatch",
			"saved_rows": saved_rows,
			"expected_rows": player_grid.size(),
		})
		return false
	...
	for r in range(rows):
		...
		if typeof(saved_row) != TYPE_ARRAY:
			if DEBUG_AP_SAVE: print_debug("[APSave] Saved row %d invalid" % r)
			continue
	...
	if DEBUG_AP_SAVE: print_debug("[APSave] Restoring revealed=%s solved=%s hidden=%s" % [str(state.get("revealed_ids", [])), str(state.get("solved_ids", [])), str(state.get("hidden_queue", []))])
	hidden_entry_queue = []
	...
	var combined_entries := _all_entries()
	...
	var saved_total_clues_variant: Variant = state.get("total_direct_clues_received", -1)
	if typeof(saved_total_clues_variant) in [TYPE_INT, TYPE_FLOAT] and int(saved_total_clues_variant) >= 0:
		total_direct_clues_received = int(saved_total_clues_variant)
		if DEBUG_AP_SAVE: print_debug("[APSave] Restored total_direct_clues=%d from save" % total_direct_clues_received)
	else:
		# Fallback: calculate from number of revealed entries
		total_direct_clues_received = revealed_set.size()
		if DEBUG_AP_SAVE: print_debug("[APSave] Calculated total_direct_clues=%d from revealed entries" % total_direct_clues_received)
	var max_direct_clues := _get_max_direct_clue_items()
	if total_direct_clues_received > max_direct_clues:
		AP_DEBUG_LOG.event("GridManager", "restore_direct_clues_clamped", {
			"saved_total": total_direct_clues_received,
			"max_total": max_direct_clues,
		})
		total_direct_clues_received = max_direct_clues
	...
	if ap_client != null:
		...
		ap_client.restore_item_tracking(saved_last_item_index, total_direct_clues_received, total_letter_hint_items_received)
		...
		if DEBUG_AP_SAVE: print_debug("[APSave] Restored indices: last_item=%d (client_now=%d prev=%d) direct_clues_seen=%d (client_now=%d prev=%d) hints_seen=%d (client_now=%d prev=%d) pending_hint_items=%d" % [saved_last_item_index, client_last, previous_last, total_direct_clues_received, client_direct_total, previous_direct_total, total_letter_hint_items_received, client_hint_total, previous_hint_total, pending_hint_items])

	_refresh_revealed_clue_buttons()
	if DEBUG_AP_SAVE: print_debug("[APSave] Restore complete revealed_count=%d unlocked=%d solved=%d hints=%d" % [revealed_entry_count, unlocked_entry_count, solved_entry_count, letter_hints_available])
	AP_DEBUG_LOG.event("GridManager", "restore_complete", {
		"revealed_count": revealed_entry_count,
		"unlocked_count": unlocked_entry_count,
		"solved_count": solved_entry_count,
		"hidden_queue_count": hidden_entry_queue.size(),
		"claimed_location_count": claimed_location_ids.size(),
		"sent_check_entry_count": sent_checks_for_entries.size(),
		"processed_clue_location_count": processed_clue_locations.size(),
		"claimed_locations": claimed_location_ids.keys(),
		"sent_check_entries": sent_checks_for_entries.keys(),
		"processed_clue_locations": processed_clue_locations.keys(),
		"total_direct_clues_received": total_direct_clues_received,
...
func _create_clue_button(text: String, direction: String, index: int, entry_id: int) -> Button:
	...
	button.add_child(_make_overlay_panel("SolvedOverlay", clue_solved_overlay_color))
	button.add_child(_make_overlay_panel("NewClueOverlay", clue_new_overlay_color))
...
func _ensure_clue_visible(direction: String, index: int) -> void:
	if index < 0:
		return
	var container := down_scroll if direction == "down" else across_scroll
	var buttons := _get_dir_buttons(direction)
...
func _on_clue_button_pressed(direction: String, index: int) -> void:
	if _suppress_list_selection:
		return
	var entries := _get_dir_entries(direction)
	var buttons := _get_dir_buttons(direction)
	if index < 0 or index >= entries.size():
		return
	_set_active_entry(entries[index])
	_focus_cell(current_cell, true)
	if index < buttons.size():
		var btn := buttons[index] as Button
		if btn != null:
			btn.release_focus()
...
func _generate_puzzle() -> void:
	if DEBUG_SETUP: print("[DEBUG] Generate button pressed")
	if is_generating:
		if DEBUG_SETUP: print("[DEBUG] Already generating, ignoring")
		return
	if DEBUG_SETUP: print("[DEBUG] Ensuring word list...")
	if not _ensure_word_list():
		if DEBUG_SETUP: print("[DEBUG] Failed to ensure word list")
		return
	if DEBUG_SETUP: print("[DEBUG] Word list loaded successfully")
	...
	if layout.is_empty() or final_count < min_required:
		if not layout.is_empty() and final_count >= 2:
			...
			_update_active_clue_message("Generation failed — try again or reduce min words", false)
...
func _setup_clue_visibility(initial_reveal_count: int) -> void:
	...
	revealed_entry_count = 0
	unlocked_entry_count = 0
	var all_entries := _all_entries()
	for entry_variant in all_entries:
		var entry: Dictionary = entry_variant
		entry["revealed"] = false
		entry_lookup[entry.get("id", -1)] = entry
	if all_entries.is_empty():
		AP_DEBUG_LOG.event("GridManager", "clue_visibility_empty")
		return
	if current_mode == PlayMode.OFFLINE:
		for entry in all_entries:
			entry["revealed"] = true
			revealed_entry_count += 1
		AP_DEBUG_LOG.event("GridManager", "clue_visibility_offline", {
			"revealed": revealed_entry_count,
	...
	for i in range(ordered_entries.size()):
		...
		entry_lookup[entry_id] = entry
	AP_DEBUG_LOG.event("GridManager", "clue_visibility_archipelago", {
		"initial_reveal": initial_reveal_count,
		"revealed": revealed_entry_count,
		"hidden_queue_count": hidden_entry_queue.size(),
		"total": all_entries.size(),
	})
...
func _set_entry_revealed(entry_id: int) -> bool:
	if not entry_lookup.has(entry_id):
		AP_DEBUG_LOG.event("GridManager", "reveal_entry_failed", {"entry_id": entry_id, "reason": "missing_entry"})
		return false
	var entry: Dictionary = entry_lookup[entry_id]
	if _entry_is_revealed(entry):
		AP_DEBUG_LOG.event("GridManager", "reveal_entry_skipped", {"entry_id": entry_id, "reason": "already_revealed"})
		return false
	entry["revealed"] = true
	...
	if current_entry_id == entry_id:
		...
		_update_active_clue_label()
	_queue_save_progress()
	AP_DEBUG_LOG.event("GridManager", "entry_revealed", {
		"entry_id": entry_id,
...
func _reveal_next_hidden_entry() -> bool:
	var revealed := false
	var before_count := hidden_entry_queue.size()
	var attempted: Array = []
	...
	AP_DEBUG_LOG.event("GridManager", "reveal_next_hidden_entry", {
		"revealed": revealed,
		"hidden_before": before_count,
		"hidden_after": hidden_entry_queue.size(),
		"attempted": attempted,
		"revealed_count": revealed_entry_count,
		"unlocked_count": unlocked_entry_count,
	})
...
func _remove_entry_from_hidden_queue(entry_id: int) -> bool:
	for i in range(hidden_entry_queue.size()):
		if int(hidden_entry_queue[i]) == entry_id:
			hidden_entry_queue.remove_at(i)
			return true
	return false

func _unveil_clue_for_out_of_logic_solve(entry: Dictionary) -> Dictionary:
	var entry_id: int = int(entry.get("id", -1))
	if entry_id <= 0 or entry.get("revealed", false):
		return entry
	var hidden_before := hidden_entry_queue.size()
	var removed_from_hidden := _remove_entry_from_hidden_queue(entry_id)
	entry["revealed"] = true
	entry["newly_revealed"] = false
	entry_lookup[entry_id] = entry
	revealed_entry_count += 1
	unlocked_entry_count += 1
	AP_DEBUG_LOG.event("GridManager", "out_of_logic_clue_unveiled", {
		"entry_id": entry_id,
		"number": entry.get("number", 0),
		"direction": entry.get("direction", ""),
		"removed_from_hidden": removed_from_hidden,
...
func _on_cell_clicked(row: int, col: int, _was_current: bool) -> void:
	...
	if DEBUG_CLICK_TRACKING:
		_debug_log_cell_click(pos, has_active, is_across, is_down, is_intersection, has_num_label, num_label, options)
...
func _on_hint_requested(row: int, col: int) -> void:
	...
	_queue_save_progress()
	if DEBUG_AP_PROCESS: print_debug("[Hint] Revealed letter '%s' at (%d, %d)" % [solution_letter, row, col])
...
func _check_entries_for_cell(pos: Vector2i) -> void:
	...
	for key in options.keys():
		...
		if now_solved and not was_solved:
			if DEBUG_AP_PROCESS: print_debug("[APCheck] Entry solved id=%d number=%d loc_idx=%d" % [entry_id, entry.get("number", 0), int(entry.get("location_index", -1))])
			entry["solved"] = true
			...
			_update_cells_for_entry(entry, true)
			if current_mode == PlayMode.ARCHIPELAGO:
				entry = _unveil_clue_for_out_of_logic_solve(entry)
...
func _update_cells_for_entry(entry: Dictionary, solved: bool) -> void:
	DEBUG_CELL_LOCK.log_word_completion(entry, self)
	...
	for cell_pos_variant in cells_list:
		...
		DEBUG_CELL_LOCK.log_cell_state(cell, "BEFORE set_state")
		
		if solved:
			cell.set_highlighted(false)
			# Set to correct state and lock if overlay is showing correct color
			cell.set_state(CrosswordCell.CellState.CORRECT, true)
		else:
			cell.set_state(CrosswordCell.CellState.NONE)
		
		DEBUG_CELL_LOCK.log_cell_state(cell, "AFTER set_state")

func _collect_unsent_solved_entries() -> Array:
	var unsent: Array = []
	for entry_variant in _all_entries():
		var entry: Dictionary = entry_variant
		var entry_id: int = int(entry.get("id", -1))
		if entry_id <= 0:
			continue
		if entry.get("solved", false) and not sent_checks_for_entries.has(entry_id):
			unsent.append(entry)
	return unsent

func _resend_unsent_solved_archipelago_checks(reason: String) -> void:
	var unsent := _collect_unsent_solved_entries()
	if unsent.is_empty():
		AP_DEBUG_LOG.event("GridManager", "resend_unsent_checks_noop", {"reason": reason})
		return
	if current_mode != PlayMode.ARCHIPELAGO or not _puzzle_ready or ap_client == null or not ap_client.is_ap_connected():
		AP_DEBUG_LOG.event("GridManager", "resend_unsent_checks_skipped", {
			"reason": reason,
			"current_mode": current_mode,
			"puzzle_ready": _puzzle_ready,
			"connected": ap_client.is_ap_connected() if ap_client else false,
			"unsent_count": unsent.size(),
			"unsent_entry_ids": unsent.map(func(entry: Dictionary) -> int: return int(entry.get("id", -1))),
		})
		return
	AP_DEBUG_LOG.event("GridManager", "resend_unsent_checks_start", {
		"reason": reason,
		"unsent_count": unsent.size(),
		"sent_check_entries": sent_checks_for_entries.keys(),
		"unsent_entry_ids": unsent.map(func(entry: Dictionary) -> int: return int(entry.get("id", -1))),
	})
	for entry_variant in unsent:
		var entry: Dictionary = entry_variant
		_send_archipelago_location_for_entry(entry)
	var remaining := _collect_unsent_solved_entries()
	AP_DEBUG_LOG.event("GridManager", "resend_unsent_checks_done", {
		"reason": reason,
...
func _send_archipelago_location_for_entry(entry: Dictionary) -> void:
	if ap_client == null or not ap_client.is_ap_connected():
		if DEBUG_AP_PROCESS: print_debug("[APCheck] Cannot send location: client=%s connected=%s" % [str(ap_client != null), str(ap_client.is_ap_connected() if ap_client else false)])
		AP_DEBUG_LOG.event("GridManager", "location_check_not_sent", {
			"reason": "disconnected",
			"client_present": ap_client != null,
			"connected": ap_client.is_ap_connected() if ap_client else false,
	...
	var entry_id: int = entry.get("id", -1)
	if entry_id < 0:
		if DEBUG_AP_PROCESS: print_debug("[APCheck] Invalid entry_id for entry: %s" % str(entry))
		AP_DEBUG_LOG.event("GridManager", "location_check_not_sent", {
	...
	if sent_checks_for_entries.has(entry_id):
		if DEBUG_AP_PROCESS: print_debug("[APCheck] Already sent check for entry_id=%d (word '%s')" % [entry_id, str(entry.get("answer", ""))])
		AP_DEBUG_LOG.event("GridManager", "location_check_not_sent", {
			"reason": "already_sent_for_entry",
			"entry_id": entry_id,
			"number": entry.get("number", 0),
		})
		return
	...
	if next_ap_id <= 0:
		if DEBUG_AP_PROCESS: print_debug("[APCheck] Invalid next_ap_id for check_index %d" % next_check_index)
		AP_DEBUG_LOG.event("GridManager", "location_check_not_sent", {
			"reason": "invalid_ap_id",
			"next_check_index": next_check_index,
		})
		return
	
	if DEBUG_AP_PROCESS: print_debug("[APCheck] Sending location check #%d (ap_id=%d) for word '%s' (clue %d, entry_id=%d)" % [next_check_index, next_ap_id, str(entry.get("answer", "")), entry.get("number", 0), entry_id])
	AP_DEBUG_LOG.event("GridManager", "location_check_send", {
		"check_index": next_check_index,
		"ap_id": next_ap_id,
		"entry_id": entry_id,
		"number": entry.get("number", 0),
		"sent_count_before": sent_checks_for_entries.size(),
	})
	if not ap_client.send_location_checks([next_ap_id]):
		AP_DEBUG_LOG.event("GridManager", "location_check_not_sent", {
			"reason": "send_failed",
			"check_index": next_check_index,
			"ap_id": next_ap_id,
...
func _on_ap_location_checked(location_ids: Array) -> void:
	AP_DEBUG_LOG.event("GridManager", "location_checked_signal", {"locations": location_ids})
	_process_ap_locations(location_ids, false)

func _on_ap_items_received(location_ids: Array) -> void:
	if not _puzzle_ready:
		AP_DEBUG_LOG.event("GridManager", "items_received_queued", {
			"locations": location_ids,
			"reason": "puzzle_not_ready",
		})
		...
		return
	AP_DEBUG_LOG.event("GridManager", "items_received", {"locations": location_ids})
...
func _on_ap_direct_clues_received(total_from_server: int, new_from_packet: int = -1) -> void:
	if not _puzzle_ready:
		AP_DEBUG_LOG.event("GridManager", "direct_clues_queued", {
			"total_from_server": total_from_server,
			"new_from_packet": new_from_packet,
			"reason": "puzzle_not_ready",
		})
		...
		return
	AP_DEBUG_LOG.event("GridManager", "direct_clues_received", {
		"total_from_server": total_from_server,
		"new_from_packet": new_from_packet,
	})
...
func _handle_direct_clue_total(total_from_server: int, new_clues_override: int = -1) -> void:
	...
	if DEBUG_AP_PROCESS: print_debug("[APDirectClues] Received packet with %d total direct clues, we had %d before" % [total_from_server, total_direct_clues_received])
	AP_DEBUG_LOG.event("GridManager", "direct_clues_process_start", {
		"total_from_server": total_from_server,
		"new_clues_override": new_clues_override,
		"total_direct_clues_received": total_direct_clues_received,
		"hidden_queue_count": hidden_entry_queue.size(),
	})
	...
	if new_clues <= 0:
		if DEBUG_AP_PROCESS: print_debug("[APDirectClues] No new direct clues (already had all %d)" % total_from_server)
		total_direct_clues_received = max(total_direct_clues_received, total_from_server)
		AP_DEBUG_LOG.event("GridManager", "direct_clues_noop", {
			"total_from_server": total_from_server,
			"total_direct_clues_received": total_direct_clues_received,
		})
		return
	...
	if revealed_count <= 0:
		if DEBUG_AP_PROCESS: print_debug("[APDirectClues] No hidden entries left to reveal")
		total_direct_clues_received = max(total_direct_clues_received, total_from_server)
		AP_DEBUG_LOG.event("GridManager", "direct_clues_no_hidden_entries", {
			"requested_new": new_clues,
			"total_from_server": total_from_server,
			"total_direct_clues_received": total_direct_clues_received,
		})
		return
	...
	_populate_clue_lists()
	_queue_save_progress()
	if DEBUG_AP_PROCESS: print_debug("[APDirectClues] Revealed %d new clues!" % revealed_count)
	if diff > 0:
		total_direct_clues_received = max(total_direct_clues_received, total_from_server)
	else:
		total_direct_clues_received = max(total_direct_clues_received + revealed_count, total_from_server)
	AP_DEBUG_LOG.event("GridManager", "direct_clues_process_done", {
		"requested_new": new_clues,
...
func _apply_new_letter_hints(count: int) -> void:
	...
	if DEBUG_AP_PROCESS: print_debug("[LetterHint] Added %d new hints! Now have %d available" % [count, letter_hints_available])
	AP_DEBUG_LOG.event("GridManager", "letter_hints_applied", {
		"count": count,
		"available": letter_hints_available,
		"total_received": total_letter_hint_items_received,
	})

func _on_ap_letter_hint_received(new_hints: int) -> void:
	# Only process if letter hints are enabled
	if not letter_hints_enabled:
		if DEBUG_AP_PROCESS: print_debug("[LetterHint] Received hints but feature is disabled, ignoring")
		AP_DEBUG_LOG.event("GridManager", "letter_hints_ignored", {"reason": "disabled", "new_hints": new_hints})
		return

	# The signal now contains only NEW hints, not total
	if DEBUG_AP_PROCESS: print_debug("[LetterHint] Received %d new hints" % new_hints)

	if ap_client == null:
		if DEBUG_AP_PROCESS: print_debug("[LetterHint] AP client missing; ignoring packet")
		AP_DEBUG_LOG.event("GridManager", "letter_hints_ignored", {"reason": "missing_client", "new_hints": new_hints})
		return
	...
	if actual_new <= 0:
		if DEBUG_AP_PROCESS: print_debug("[LetterHint] All hints already applied (server_total=%d accounted=%d)" % [server_total, accounted])
		AP_DEBUG_LOG.event("GridManager", "letter_hints_noop", {
			"server_total": server_total,
			"accounted": accounted,
			"new_hints": new_hints,
		})
		return
	if DEBUG_AP_PROCESS: print_debug("[LetterHint] Resolving %d new hints (server_total=%d accounted=%d)" % [actual_new, server_total, accounted])

	if not _puzzle_ready:
		_pending_letter_hints += actual_new
		if DEBUG_AP_PROCESS: print_debug("[LetterHint] Puzzle not ready; queued %d hints (pending=%d)" % [actual_new, _pending_letter_hints])
		AP_DEBUG_LOG.event("GridManager", "letter_hints_queued", {
			"actual_new": actual_new,
			"pending": _pending_letter_hints,
		})
		return

	_apply_new_letter_hints(actual_new)

func _apply_cell_state_from_correctness(cell: CrosswordCell, correct: bool, lock_if_correct: bool = false) -> int:
	if cell == null:
	...
	if current_mode == PlayMode.ARCHIPELAGO:
		if has_color_indicator:
			...
			should_lock = lock_if_correct  # Only lock when word is complete and indicator is enabled
		else:
			target_state = CrosswordCell.CellState.NONE
			should_lock = false  # Never lock without color indicator
	else:
		# Offline mode: always show colors and lock completed words
		target_state = CrosswordCell.CellState.CORRECT if correct else CrosswordCell.CellState.INCORRECT
		should_lock = lock_if_correct
	
	# Apply state with appropriate locking
	if target_state == CrosswordCell.CellState.CORRECT and not should_lock:
		# Show correct overlay but don't lock the cell
		cell.set_state(target_state, false)
	else:
		cell.set_state(target_state, should_lock)
	return target_state
...
func _process_ap_locations(location_ids: Array, unlock_clues: bool, force: bool = false) -> void:
	if not force and current_mode != PlayMode.ARCHIPELAGO:
		AP_DEBUG_LOG.event("GridManager", "process_locations_skipped", {
			"reason": "not_archipelago_mode",
			"locations": location_ids,
			"unlock_clues": unlock_clues,
			"force": force,
		})
		return
	if location_ids.is_empty():
		AP_DEBUG_LOG.event("GridManager", "process_locations_skipped", {"reason": "empty_locations"})
		return
	AP_DEBUG_LOG.event("GridManager", "process_locations_start", {
	...
	for loc_variant in location_ids:
		...
		if processed_clue_locations.has(loc_id):
			if DEBUG_AP_PROCESS: print_debug("[APProcess] Skipping already processed clue location %d" % loc_id)
			continue
		new_locations.append(loc_id)
	
	if new_locations.is_empty():
		if DEBUG_AP_PROCESS: print_debug("[APProcess] No new clue locations to process")
		AP_DEBUG_LOG.event("GridManager", "process_locations_noop", {"reason": "all_processed_or_invalid"})
		return
	
	if DEBUG_AP_PROCESS: print_debug("[APProcess] Processing %d new clue locations" % new_locations.size())
	...
	if new_location_logged:
		_queue_save_progress()
	AP_DEBUG_LOG.event("GridManager", "process_locations_done", {
		"new_location_count": new_locations.size(),
...
func _report_word_list_loaded(label: String) -> void:
	var wl_all: Array = word_list.get_all_entries()
	var wl_count := wl_all.size() if typeof(wl_all) == TYPE_ARRAY else 0
	_update_active_clue_message("%s (%d entries)" % [label, wl_count])

func _get_dir_buttons(direction: String) -> Array:
	return down_buttons if direction == "down" else across_buttons

func _get_dir_entries(direction: String) -> Array:
	return down_entries if direction == "down" else across_entries

func _make_overlay_panel(overlay_name: String, color: Color) -> Panel:
...
func _on_regenerate_pressed() -> void:
	if DEBUG_SETUP: print_debug("[Generate] Button pressed!")
	_status_locked = false
	_generate_puzzle()
...
func _ensure_word_list() -> bool:
	...
	if word_list != null:
		...
		if DEBUG_SETUP: print("[DEBUG] Loaded word list from configured path: %s" % word_list_path)
		_report_word_list_loaded("Loaded word list")
		return true

	# Fallback to default path if the configured path fails
	if word_list_path != DEFAULT_WORD_LIST_PATH:
		if DEBUG_SETUP: print("Failed to load word list from '%s', trying default path '%s'" % [word_list_path, DEFAULT_WORD_LIST_PATH])
		word_list = WORD_LIST.from_file(DEFAULT_WORD_LIST_PATH)
		if word_list != null:
			_using_minimal_word_list = false
			if DEBUG_SETUP: print("[DEBUG] Loaded word list from default path: %s" % DEFAULT_WORD_LIST_PATH)
			_report_word_list_loaded("Loaded word list")
			return true

	# Final fallback: try loading from user:// directory (in case file was copied there)
	var user_path := "user://crossword_wordlist.txt"
	if DEBUG_SETUP: print("Failed to load word list from res://, trying user path '%s'" % user_path)
	word_list = WORD_LIST.from_file(user_path)
	if word_list != null:
		_using_minimal_word_list = false
		if DEBUG_SETUP: print("Successfully loaded word list from user directory")
		_report_word_list_loaded("Loaded word list")
		return true
	...
	if DEBUG_SETUP: print("Creating minimal fallback word list")
	word_list = _create_minimal_word_list()
	_using_minimal_word_list = word_list != null
	if _using_minimal_word_list:
		_report_word_list_loaded("Using fallback word list")
...
func _sort_entry_lists() -> void:
	across_entries.sort_custom(_entry_number_less)
	down_entries.sort_custom(_entry_number_less)

func _format_clue_display(number: int, clue: String) -> String:
	return "%d. %s" % [number, clue.strip_edges()]


func _connect_ap_input_submit(input: LineEdit) -> void:
	if input == null:
		return
	if not input.text_submitted.is_connected(_on_ap_input_submitted):
		input.text_submitted.connect(_on_ap_input_submitted)

func _web_store(key: String, value: String) -> void:
	if OS.has_feature("web"):
		JavaScriptBridge.eval("localStorage.setItem(%s, %s)" % [JSON.stringify(key), JSON.stringify(value)])

func _web_load(key: String) -> String:
	if OS.has_feature("web"):
		var result = JavaScriptBridge.eval("localStorage.getItem(%s) ?? ''" % JSON.stringify(key))
		return str(result) if result != null else ""
	return ""

func _save_ap_settings() -> void:
	if ap_host_input != null:
		_web_store("ap_host", ap_host_input.text)
	if ap_port_input != null:
		_web_store("ap_port", ap_port_input.text)
	if ap_name_input != null:
		_web_store("ap_name", ap_name_input.text)
	if ap_password_input != null:
		_web_store("ap_password", ap_password_input.text)

func _load_ap_settings() -> void:
	var host := _web_load("ap_host")
	var port := _web_load("ap_port")
	var name := _web_load("ap_name")
	var password := _web_load("ap_password")
	if not host.is_empty() and ap_host_input != null:
		ap_host_input.text = host
	if not port.is_empty() and ap_port_input != null:
		ap_port_input.text = port
	if not name.is_empty() and ap_name_input != null:
		ap_name_input.text = name
	if not password.is_empty() and ap_password_input != null:
		ap_password_input.text = password

func _on_ap_input_submitted(_text: String) -> void:
	_on_ap_connect_pressed()

func _on_ap_connect_pressed() -> void:
	AP_DEBUG_LOG.event("GridManager", "connect_button_pressed", {
		"current_mode": current_mode,
		"client_state": ap_client.get_state() if ap_client != null else -1,
	})
	...
	if name_text.is_empty():
		...
		AP_DEBUG_LOG.event("GridManager", "connect_input_invalid", {"reason": "missing_player_name"})
		_update_ap_button_text()
		return
	...
	if port_text.is_empty():
		...
		AP_DEBUG_LOG.event("GridManager", "connect_input_invalid", {"reason": "missing_port"})
		_update_ap_button_text()
		return
	...
	if port <= 0:
		...
		AP_DEBUG_LOG.event("GridManager", "connect_input_invalid", {"reason": "invalid_port", "port_text": port_text})
		_update_ap_button_text()
		return
...
func _enter_offline_mode(generate_puzzle: bool = true) -> void:
	AP_DEBUG_LOG.event("GridManager", "enter_offline_mode", {
...
func _enter_archipelago_mode() -> void:
	if current_mode == PlayMode.ARCHIPELAGO:
		return
	AP_DEBUG_LOG.event("GridManager", "enter_archipelago_mode", {
		"has_slot_data": not ap_slot_data.is_empty(),
		"client_state": ap_client.get_state() if ap_client != null else -1,
	})
